...
* Prepare database (IHM) :
  ...
  - Load GFA data : select GFA files to load and click on "Generate CSV Import file" button. According to your data, there are 2 possibilities :
    - Unique GFA with multiple chromosomes : in this case the chromosome must be defined in the path / walk according to the rules described in "Important notes" section. There is no chromosome to set into input box associated to the file.
  ...
  - Create the database : give a name to the neo4J container and then click on "Create new DB" button. This step will use the csv generated by the previous step in the /data/import directory.
  - Load annotations by selecting the files to load (gtf or gff) and, for each selected file, the genome associated. Before to load annotations it is required that indexes are fully created : after creating data or loading GFA the indexes are automatically created but if data are big it requires some time, in this case a warning message is displayed.
...
* There are 3 ways to generate database :
    ...
    - From the GFA file (and gtf / gff if available) : the database is constructed directly from the GFA file. This is the classical procedure to create a new database. It is recommended to follow the steps described in the "Quickstart" section and to use the csv import procedure. But if it is required to add data after you can select a gfa file and click on the "Add data" button (this procedure can take a long time and is not recommended for big gfa files).
